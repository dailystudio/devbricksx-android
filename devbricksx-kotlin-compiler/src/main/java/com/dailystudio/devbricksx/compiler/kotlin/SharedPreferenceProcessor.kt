    ...
    private fun generateSharedPreference(element: TypeElement) : GeneratedResult? {
        ...
        for (subElement in subElements) {
            if (subElement is VariableElement) {
                debug("processing field: %s[%s], constant value: %s",
                        subElement, subElement.asType().asTypeName(), subElement.constantValue)
                ...
                val propBuilder = PropertySpec.builder(varName,
                        TypeNamesUtils.javaToKotlinTypeName(fieldType.asTypeName()))
                        .mutable()
