...
public class SequencerImpl implements Sequencer {
            ...
            while (isOpen) {
                ...
                for (int loop = 0; loop < getLoopCount() + 1; loop = (getLoopCount() == LOOP_CONTINUOUSLY ? loop : loop + 1)) {
                    ...
                    for (int i = 0; i < playingTrack.size(); i++) {
                        ...
                        synchronized (receivers) {
                            for (final Receiver receiver : receivers) {
                                receiver.send(midiMessage, midiEvent.getTick());
                            }
                        }
    ...
    private SoftSynthesizer synthesizer = null;

    private Receiver externalReceiver = null;
    /**
     * Constructor
     */
    ...
    public SequencerImpl(SoftSynthesizer synthesizer) {
    ...
    public void open() throws MidiUnavailableException {
        ...
        synchronized (receivers) {
            ...
            if (synthesizer != null && receivers.isEmpty()) {
                receivers.add(new SoftReceiver(synthesizer));
            }

            if (externalReceiver != null) {
                receivers.add(externalReceiver);
            }
        }
