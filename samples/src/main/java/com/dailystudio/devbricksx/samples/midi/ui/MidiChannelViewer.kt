package com.dailystudio.devbricksx.samples.midi.ui
...
import com.dailystudio.devbricksx.samples.midi.utils.ChannelInfo
import com.dailystudio.devbricksx.samples.midi.utils.TrackInfo
...
//            Logger.debug("draw rect for tick [${pair.first}]: ${pair.second}")
    ...
    fun displayEvents(
        ...
        for ((i, event) in displayEvents.withIndex()) {
            ...
            Logger.debug("message [$i, tick: ${event.tick}]: ${message.command.toHexString()}, ${message.data1.toHexString()}, ${message.data2.toHexString()}")
        }
